...
// Fill color lives in widget.html (.fill) — surface is always dark, so it's theme-independent.
...
function render(s: any): void {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const reset = document.getElementById(`reset-${i}`)!;
    const label = document.getElementById(`label-${i}`)!;
    // Monthly-only providers (Copilot, Cursor) have one bucket: drop the second row
    // rather than showing a dead "7d –".
    track.parentElement!.style.display = !bucket && i > 0 ? "none" : "";
    // Gemini rows are per model, all daily: show the model ("Pr"/"Fl") instead of a duplicate "1d".
    label.textContent = bucket
      ? bucket.kind === "gemini_daily"
        ? (bucket.label.split("· ")[1] ?? "1d").slice(0, 2)
        : SHORT_LABEL[bucket.kind] ?? "mo"
      : i === 0 ? "5h" : "7d";
    label.title = bucket?.label ?? "";
    track.setAttribute("aria-label", bucket?.label ?? "");
    if (bucket) {
