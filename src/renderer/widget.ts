...
// Provider glyphs, keyed by adapter id. Claude carries its own color; Codex is
// drawn in the surface text color (the widget surface is always dark).
...
function render(s: any): void {
  ...
  for (const kind of ["session", "weekly_all"]) {
    const bucket =
      s.quota?.status === "ok" || s.quota?.buckets?.length
        ? s.quota.buckets.find((b: any) => b.provider === prov && b.kind === kind)
        : null;
    const track = document.getElementById(`track-${kind}`)!;
